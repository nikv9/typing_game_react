...
import GameOverMsg from "../components/GameOverMsg";
...
const Game = () => {
  ...
  const getRandomWord = () => {
    setPowerUp(
      Math.random() < 0.1
        ? ["FREEZE", "BLAST", "HEAL"][Math.floor(Math.random() * 3)]
        : null,
    );
  ...
  const calculateStats = () => {
    if (stats.current.totalChars > 0) {
      setAccuracy(
        Math.round(
          (stats.current.correctChars / stats.current.totalChars) * 100,
        ),
      );
    }
  };
  ...
  const handleTyping = (e) => {
    ...
    if (val.length > inputVal.length) {
      ...
      if (val[val.length - 1] === currentWord.toLowerCase()[val.length - 1])
        stats.current.correctChars++;
    }
    setInputVal(val);
    calculateStats();
    ...
    <div className="h-dvh bg-[radial-gradient(circle_at_center,_#f8fafc_0%,_#ffffff_100%)] text-slate-900 relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none opacity-20 z-0">
        {[...Array(20)].map((_, i) => (
          <div
            key={i}
            className="bg-particle w-1 h-1 bg-emerald-200/20 blur-[1px]"
            style={{
        ...
        {!start ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-8 font-sans px-6">
            <div className="text-center space-y-2">
              <h2 className="text-5xl font-black tracking-widest text-slate-800/20 uppercase">
                Ready?
              </h2>
              <p className="text-slate-500 text-sm tracking-widest font-bold">
                Speed and accuracy are key.
              </p>
            </div>
            <button
              className="group relative px-12 py-6 bg-emerald-600 text-white font-black tracking-[0.3em] uppercase rounded-3xl hover:bg-emerald-500 hover:scale-110 active:scale-95 transition-all shadow-lg ring-1 ring-white/20"
              onClick={startGame}
            >
      ...
      {gameOver && (
        <GameOverMsg score={score} stats={{ accuracy }} onRestart={startGame} />
      )}
    </div>
  );
};
